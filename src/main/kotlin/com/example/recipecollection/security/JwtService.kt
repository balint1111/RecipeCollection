...
class JwtService(
    ...
    fun extractUsername(token: String): String = parseToken(token).payload.subject

    fun extractRoles(token: String): List<String> {
        return when (val roles = parseToken(token).payload["roles"]) {
    ...
    fun isTokenValid(
        token: String,
        username: String,
    ): Boolean = extractUsername(token) == username
